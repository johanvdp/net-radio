...
uint32_t buffer_free() {
	return CONFIG_SPI_RAM_TOTAL_BYTES - (buffer_write_addr - buffer_read_addr);
}

void buffer_push(uint8_t *data, uint32_t length) {
	assert(buffer_free() >= length);
	spi_ram_data_write(buffer_write_addr, length, data);
...
void buffer_pull(uint8_t *page, uint32_t length) {
	...
	spi_ram_data_read(buffer_read_addr, length, page);
